...
3) Complete backend CRUD
	...
	[3.9] Modify Controllers
		...
		#ImageController
			...
			#app->Http->Controllers->ImageController.ph
				<?php
...
/*	Edit Image Issue:
		...
		Edit image will not work because dropzone does not support put request.
		It only uses Post request, which is why we need to use the approach same as profile picture.
		We will create a new table for post image same user images and fetch the most recent image.
		Seperate table will also allow us create an album.
		We can create colums such as 'id', 'image', 'type' (single or album).
		If it is single, we can fetch the recent one in resource.
		If it is album we willfetch all.
*/